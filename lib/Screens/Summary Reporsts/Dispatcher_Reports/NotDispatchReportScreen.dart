...
class NotDispatchReportScreen extends StatefulWidget {
  const NotDispatchReportScreen({Key? key}) : super(key: key);

  @override
  _NotDispatchReportScreenState createState() =>
      _NotDispatchReportScreenState();
}

class _NotDispatchReportScreenState extends State<NotDispatchReportScreen> {
  List<Map<String, dynamic>> records = [];
  List<String> allColumns = [];
  bool isLoading = true;
  String errorMessage = '';

  // Same color scheme as DispatchReportScreen
  static const Color _themeColor = Colors.blueGrey;
  ...
  Future<void> fetchRecords() async {
    try {
      ...
      final String url =
          'https://cloud.metaxperts.net:8443/erp/valor_trading/dispatchmastern/get/$user_id';
      debugPrint('🔗 Fetching not-dispatched from: $url');
      ...
      debugPrint('📊 Not-Dispatch API Status: ${response.statusCode}');
      ...
      if (response.statusCode == 200) {
        ...
        // Process all rows — no status filter needed (API already returns not-dispatched)
        ...
        // Build ordered column list — priority columns first
        ...
        debugPrint('✅ Loaded ${records.length} not-dispatched records');
        debugPrint('📋 Columns: $allColumns');
      } else {
        throw Exception('HTTP ${response.statusCode}: ${response.body}');
      }
    } catch (e) {
      debugPrint('❌ Not-Dispatch API Error: $e');
      setState(() {
  ...
  // ====================== DETAIL BOTTOM SHEET ======================
  void showDetailsDialog(Map<String, dynamic> row) {
    ...
    final shopName =
    (row['shop_name'] ?? row['SHOP_NAME'] ?? 'N/A').toString();
    ...
    if (isStatus) {
      valueWidget = Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: _statusColor(value).withOpacity(0.4)),
        ),
        child: Text(
          value,
          style: TextStyle(
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const Text('Loading not-dispatched records...'),
          ],
        ),
      );
    }
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            const Text('No not-dispatched records found',
                ...
                cells: [
                  ...
                  DataCell(
                    IconButton(
                      ...
                      onPressed: () => showDetailsDialog(row),
