...
//         debugPrint(
//             "⚠️ Central points already exist for today, deleting old ones...");
//
...
//
//           var result = await clusterDetector.processPointWithTime(
//               latLng, point.time!);
//
...
//             debugPrint(
//                 "   Time Observed: ${result['time_observed_minutes']} minutes");
//           }
//
...
//         debugPrint(
//             "   🔸 Cluster ${cluster['key']}: ${cluster['points']} points");
//       }
//
...
//   Future<Map<String, dynamic>> processSinglePoint(LatLng point,
...
//   Future<void> _storeSingleCluster(String clusterKey,
//       LatLng center,
...
//       String basicAddress = pureAddress
//           .split('\n')
//           .first;
//
//       String uniqueClusterId = centralPointsRepo.generateCentralPointId();
//
//       double stayTime = await _calculateClusterStayTime(
//           clusterPoints, timestamp);
...
//   """);
//
//       CentralPointsModel clusterRecord = CentralPointsModel
//           .createIndividualCluster(
//         mainCentralPointId: "REALTIME-${DateFormat('yyyyMMdd').format(
...
//         clusterAddress: basicAddress,
//         // Sirf basic address pass karein
...
//   }
//
//   Future<double> _calculateClusterStayTime(List<LatLng> points,
//       DateTime referenceTime) async {
...
//       debugPrint("📏 Cluster Radius: ${clusterDistanceMeters.toStringAsFixed(
//           0)} meters");
...
//         Get.snackbar(
//           'No Clusters',
//           'No confirmed clusters found within ${clusterDistanceMeters
//               .toStringAsFixed(0)} meters distance',
//           backgroundColor: Colors.orange,
//         );
//         return;
//       }
//
//       String mainProcessingId = "INTELLIGENT-${DateFormat('yyyyMMdd').format(
//           DateTime.now())}-$user_id";
//
...
//
//         String clusterAddress = await _getExtremelyDetailedAddressFromLatLng(
//             center);
//
//         String uniqueClusterId = centralPointsRepo.generateCentralPointId();
//
...
//       debugPrint(
//           "💾 Saved ${allClusterRecords.length} confirmed cluster records");
//
//       int successfullyPosted = 0;
//       for (int i = 0; i < allClusterRecords.length; i++) {
//         var cluster = allClusterRecords[i];
//         debugPrint(
//             "🔄 Posting Cluster ${i + 1}/${allClusterRecords.length} to API...");
//         bool apiSuccess = await centralPointsRepo.postCentralPointToAPI(
//             cluster);
//
//         if (apiSuccess) {
...
//       Get.snackbar(
//         'Success',
//         'Created ${clusterCenters
//             .length} confirmed clusters with ${clusterDistanceMeters
//             .toStringAsFixed(0)}m radius',
...
//           'lat': 33.6843,
//           'lng': 73.0478,
//           'time': DateTime.now().subtract(Duration(minutes: 26))
//         },
//         {
//           'lat': 33.6846,
//           'lng': 73.0481,
...
//           'lat': 33.6844,
//           'lng': 73.0479,
//           'time': DateTime.now().subtract(Duration(minutes: 22))
//         },
//
...
//
//         addressParts.add(
//             "Coordinates: ${point.latitude.toStringAsFixed(6)}, ${point
//                 .longitude.toStringAsFixed(6)}");
//
//         addressParts.add(
//             "Cluster Radius: ${clusterDistanceMeters.toStringAsFixed(
//                 0)} meters");
//
//         String currentTime = DateFormat('HH:mm:ss').format(DateTime.now());
//         addressParts.add("Processed: $currentTime");
//
//         addressParts.add("Note: ${clusterDistanceMeters.toStringAsFixed(
//             0)}m cluster distance");
//
...
//
//     String currentTime = DateFormat('HH:mm:ss').format(DateTime.now());
//     return """Coordinates: ${point.latitude.toStringAsFixed(6)}, ${point
//         .longitude.toStringAsFixed(6)}
// Cluster Radius: ${clusterDistanceMeters.toStringAsFixed(0)} meters
...
//       {double timeThresholdMinutes = 1.0}) async {
...
//         double distanceMeters = _calculateHaversineDistanceMeters(
...
//           debugPrint(
//               "🟢 Cluster $clusterKey: Entry at ${DateFormat('HH:mm:ss').format(
//                   currentTime)}");
//           continue;
//         }
//
...
//             coordinates, clusterDistanceMeters: distance);
//         debugPrint("   📏 ${distance.toStringAsFixed(0)}m -> ${clusters
//             .length} clusters");
//       }
//     } catch (e) {
...
//
//       var clusters = gpxService.clusterCoordinates(
//           coordinates, clusterDistanceMeters: clusterDistanceMeters);
...
//               .longitude}");
//       debugPrint(
//           "   - Cluster Distance: ${clusterDistanceMeters.toStringAsFixed(
//               0)} meters");
//
//       await storeCentralPointsToAPI(
//           overallCenter,
