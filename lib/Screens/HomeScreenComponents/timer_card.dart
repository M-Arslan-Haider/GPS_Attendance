...
class _TimerCardState extends State<TimerCard> with WidgetsBindingObserver {
  ...
  Future<void> _syncCriticalEventData({
    ...
    try {
      ...
      // ✅ FIX: Read employee data safely (handles int/String stored types)
      ...
      // ✅ FIX: Pass empId explicitly so attendance_out record is not empty
  ...
  Future<void> _handleAutoClockOut({
    ...
    try {
      ...
      // ✅ FIX: Read empId safely (handles int/String stored types)
      final String empId = _safePrefsString(prefs, 'emp_id');
  ...
  Future<void> _handleClockIn(BuildContext context) async {
    ...
    try {
      ...
      // 🔍 DEBUG: Print ALL SharedPreferences keys so we can confirm exact key names
      final Set<String> allPrefsKeys = prefs.getKeys();
      debugPrint('🔑 [PREFS DEBUG] All saved keys (' + allPrefsKeys.length.toString() + '):');
      for (final k in allPrefsKeys) {
        debugPrint('   KEY: "' + k + '" = "' + (prefs.get(k)?.toString() ?? 'null') + '"');
      }

      // ✅ FIX: Read emp_id (stored as int by login), emp_name and job (stored as String).
      // _safePrefsString uses prefs.get() which returns dynamic, then .toString() —
      // so it works regardless of the stored type (int, String, etc.).
      final String empId = _safePrefsString(prefs, 'emp_id');

      // emp_name: try the key that LoginModels uses, then common alternatives
      final String empName = _safePrefsStringFallback(prefs, [
        'emp_name', 'empName', 'employee_name', 'name', 'userName', 'user_name',
      ]);

      // job: try the key that LoginModels uses, then common alternatives
      ...
      // 5. ✅ FIX: FAST CLOCK IN - Pass all employee fields
  ...
  Future<void> _handleClockOut(BuildContext context) async {
    ...
    try {
      ...
      // ✅ FIX: Read employee ID safely (handles int/String stored types)
      final String empId = _safePrefsString(prefs, 'emp_id');
      debugPrint('👤 [CLOCK-OUT] empId=$empId');
      ...
      // 9. ✅ FIX: SAVE IN BACKGROUND — pass empId explicitly
...
<gpx version="1.1" creator="AttendanceApp">
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // SAFE PREFS READERS
  // Uses prefs.get() (returns dynamic) instead of getString() to avoid the
  // ClassCastException thrown when a key was saved as int (e.g. emp_id).
  // ──────────────────────────────────────────────────────────────────────────
  ...
  /// This handles cases where the login screen saves under a different key name
  /// than what the attendance screen expects (e.g. 'empName' vs 'emp_name').
                ...
                Obx(() => SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                      ...
                      disabledBackgroundColor: const Color(0xFFEBEEFD),
                      elevation: attendanceViewModel.isClockedIn.value ? 0 : 4,
                      shadowColor: const Color(0xFF4354E8).withOpacity(0.35),
                      shape: RoundedRectangleBorder(
                ...
                Obx(() => SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                      ...
                      elevation: attendanceViewModel.isClockedIn.value ? 4 : 0,
