...
// //       onCreate: _onCreate,
// //       onUpgrade: _onUpgrade,    // Added upgrade function
// //     );
// //
// //     return db;
// //   }
...
// //     ];
// //
// //     debugPrint('✅ All tables created successfully - Version 9');
// //
...
// //       final db = await this.db;
// //
// //       // Generate leave ID in format: LV-USERID-DATE-SEQUENCE
// //       // Format: LV-TEST0001-04-Dec-003
...
// //         'reason': leave.reason,
// //         'attachment_url': leave.attachmentUrl ?? '',
// //         'application_date': DateTime.now().toIso8601String().split('T')[0],
// //         'application_time': "${DateTime.now().hour}:${DateTime.now().minute}",
// //         'status': leave.status ?? 'pending',
// //         'posted': 0,
// //       };
// //
// //       debugPrint('📝 Inserting leave to database: $data');
...
// //   }
// //
// // // Helper method to get sequence number for the day
// //   Future<int> _getLeaveSequenceForDay(DateTime date) async {
// //     try {
// //       final db = await this.db;
// //       String dateStr = date.toIso8601String().split('T')[0]; // YYYY-MM-DD format
// //
// //       // Count how many leaves were created today by this user
...
// //   //   try {
// //   //     final db = await this.db;
// //   //     String leaveId = leave.id ?? 'LV${DateTime.now().millisecondsSinceEpoch}${leave.bookerId.substring(0, 3)}';
// //   //
// //   //     final data = {
// //   //       'id': DateTime.now().millisecondsSinceEpoch.toString(),
// //   //       'leave_id': leaveId,
...
// //   //     debugPrint('📝 Inserting leave to database: $data');
// //   //     final result = await db.insert(leaveTable, data, conflictAlgorithm: ConflictAlgorithm.replace);
// //   //     debugPrint('✅ Leave inserted with ID: $result');
// //   //     return result;
// //   //   } catch (e) {
// //   //     debugPrint('❌ Error inserting leave: $e');
// //   //     return 0;
...
// //       } else {
// //         debugPrint('⚠ No leave found with ID: $leaveId');
// //       }
// //
// //       return result;
// //     } catch (e) {
...
//     ];
//
//     debugPrint('✅ All tables created successfully - Version 10');
//
//     for (var query in tableQueries) {
//       await db.execute(query);
//     }
...
//       String leaveId = 'LV-${leave.bookerId}-$day-$monthAbbrev-$sequenceStr';
//
//       // Get attachment bytes from model (already processed in LeaveFormScreen)
//       List<int>? attachmentBytes = leave.attachmentData;
//
//       final data = {
//         'id': DateTime.now().millisecondsSinceEpoch.toString(),
//         'leave_id': leaveId,
...
//         'is_half_day': leave.isHalfDay ? 1 : 0,
//         'reason': leave.reason,
//         'attachment_data': attachmentBytes, // BLOB data
//         'application_date': DateTime.now().toIso8601String().split('T')[0],
//         'application_time': "${DateTime.now().hour}:${DateTime.now().minute}",
//         'status': leave.status ?? 'pending',
//         'posted': 0,
...
//       debugPrint('   Attachment size: ${attachmentBytes?.length ?? 0} bytes');
//
//       final result = await db.insert(leaveTable, data, conflictAlgorithm: ConflictAlgorithm.replace);
//       debugPrint('✅ Leave inserted with ID: $result, Leave ID: $leaveId');
//       return result;
...
class DBHelper extends GetxService {
  ...
  initDatabase() async {
    ...
    var db = openDatabase(
      path,
      version: 13, // Increased to 13 for leave table updates
      onCreate: _onCreate,
      onUpgrade: _onUpgrade,
    );
  ...
  _onCreate(Database db, int version) async {
    ...
    debugPrint('✅ All tables created successfully - Version 13');
  ...
  _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 13) {
      try {
        ...
        debugPrint('✅ Database upgraded to version 13 successfully');
      } catch (e) {
